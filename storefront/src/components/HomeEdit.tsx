...
export function ListEdit({ kind }: { kind: ListKey }) {
  ...
  const aiTr = async (i: number, src: string, dst: string, long = false) => { const v = String(rows[i][src] || "").trim(); if (!v) return; const r = await translateOne(v, long); if (r) upd(i, { [dst]: r }); };
